...
import type { ILoanRepository } from '../../../src/domain/repositories/loan-repository';
import { LoanStatus } from '../../../src/domain/types/loan-status';
import { CreateLoanUseCase } from '../../../src/application/use-cases/create-loan.use-case';

const mockRepository: ILoanRepository = {
  save: jest.fn().mockResolvedValue(undefined),
  findById: jest.fn(),
  updateStepFunctionArn: jest.fn().mockResolvedValue(undefined),
};

const mockLoanWorkflowStarter: ILoanWorkflowStarter = {
  startLoanWorkflow: jest.fn().mockResolvedValue('arn:aws:states:::execution/test/run1'),
};
...
describe('CreateLoanUseCase', () => {
  ...
  it('saves the loan and starts a Step Functions execution', async () => {
    const useCase = new CreateLoanUseCase(mockRepository, mockLoanWorkflowStarter);
    ...
    expect(mockLoanWorkflowStarter.startLoanWorkflow).toHaveBeenCalledTimes(1);
    expect(mockRepository.updateStepFunctionArn).toHaveBeenCalledTimes(1);
  });
});
