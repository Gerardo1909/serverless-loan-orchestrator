...
function makeRepository(loan: (typeof BASE_LOAN) | null): ILoanRepository {
  return {
    ...
    updateStepFunctionArn: jest.fn(),
  };
}

describe('GetLoanStatusUseCase', () => {
  it('returns status fields when loan exists', async () => {
    const loan = { ...BASE_LOAN, stepFunctionExecutionArn: 'arn:aws:states:::execution/test' };
  ...
  it('propagates repository errors', async () => {
    const repository: ILoanRepository = {
      save: jest.fn(),
      findById: jest.fn().mockRejectedValue(new Error('DynamoDB unavailable')),
      updateStepFunctionArn: jest.fn(),
    };
    const useCase = new GetLoanStatusUseCase(repository);

    await expect(useCase.execute('loan-123')).rejects.toThrow('DynamoDB unavailable');
  });
});
